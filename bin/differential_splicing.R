...
if(!exists("ECS")){
  ...
  fitExpToVar <- as.character(design[design$exp_name==exp_name,]$fitExpToVar)
  formulaFullModel <- as.character(design[design$exp_name==exp_name,]$formulaFullModel)
  formulaReducedModel <- as.character(design[design$exp_name==exp_name,]$formulaReducedModel)
  ...
  ## itermediate save
  #print("Save R session before DEXSeq data creation:")
  #save.image(paste(paste(working_dir, "/", 
  #                       project_name, "_", 
  #                       exp_name,"_","loaded_data", ".RData", sep="")))
  
  print("Creating DEXSeq data object:") 
  ECS = DEXSeqDataSet(
    countData=countData,
    sampleData=sampleData.sorted,
    design=as.formula(formulaFullModel),
  ...
  ECS = estimateDispersions(
    ECS,
    formula=as.formula(formulaFullModel),
    BPPARAM=BPPARAM)
  
  print("Test for differential expression:")
  ECS = testForDEU(
    ECS,
    reducedModel=as.formula(formulaReducedModel),
    fullModel=as.formula(formulaFullModel),
    BPPARAM=BPPARAM)
